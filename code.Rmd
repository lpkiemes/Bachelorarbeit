...
pacman::p_load(haven, readr, tidyverse, gtsummary, ggsci, ROSE, labelled, matrixStats, car, psych, lsr, modelsummary, olsrr, moments)
...
ggplot2::ggsave(here::here("plots", "boxplot.pdf"), dpi = dpi, width = 7, height = 4, units = "in")
...
ggplot(data = allbus2006, mapping = aes(x = split, y = immigration)) +
  ...
  scale_y_continuous(
    breaks = c(1:7),
    minor_breaks = seq(0.5, 6.5, 0.5),
    labels = c("ausländerfeindlich - 1", 2:6, "ausländerfreundlich - 7")) +
  labs(
    title = "Einstellung gegenüber Immigration nach Befragungsmodus",
    subtitle = "N = 2635",
    y = "Einstellungen gegenüber Immigration",
...
#         age_cen,
#       ), 
#     method = "spearman") # all independent variables

# for (a in 2:length(colnames(allbus2006))) {
#   for (b in 3:length(colnames(allbus2006))) {
#     cor.test(x = as.numeric(allbus2006[, a]), y = as.numeric(allbus2006[, b]), method = "spearman")
#   }
# }
```
...
```{r m3}
m3.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex + int_sex * split + int_agediff + int_agediff * split + int_agediff + int_agediff * split + int_exp + int_exp * split + int_edu + int_edu * split, data = allbus2006)

summary(m3.lm)
```

```{r m4}
m4.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex + int_sex * split + int_agediff + int_agediff * split + int_agediff + int_agediff * split + int_exp + int_exp * split + int_edu + int_edu * split + sdtendency + sdperception, data = allbus2006)
...
```{r m3_nohilfe}
m3_nohilfe.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex + int_sex * split + int_agediff + int_agediff * split + int_agediff + int_agediff * split + int_exp + int_exp * split + int_edu + int_edu * split, data = allbus2006_nohilfe)

summary(m3_nohilfe.lm)
```

```{r m4_nohilfe}
m4_nohilfe.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex + int_sex * split + int_agediff + int_agediff * split + int_agediff + int_agediff * split + int_exp + int_exp * split + int_edu + int_edu * split + sdtendency + sdperception, data = allbus2006_nohilfe)
...
pairs(allbus2006 |> select(
  immigration, split, age_cen, edu, sex, int_sexdiff, int_agediff, int_agecen, sdtendency, sdperception
