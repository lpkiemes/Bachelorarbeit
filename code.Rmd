...
# 1 völlig in Ordnung
# 2 eher in Ordnung
# 3 eher nicht in Ordnung
# 4 überhaupt nicht in Ordnung
# 9 Keine Angabe

# Frage
# Ein Gastwirt weigert sich, in seinem Lokal Ausländer zu bedienen. (-)
# Eltern verbieten Ihrer 17jährigen Tochter die Freundschaft mit einem türkischen Jugendlichen. (-)
# Ein Unternehmer, der Personal abbauen muss, entlässt zuerst die ausländischen Arbeitnehmer. (-)
...
allbus2006 |> 
  group_by(v81, meinung_wirt) |> 
  summarise(n())

allbus2006 |> 
  group_by(v82, meinung_eltern) |> 
  summarise(n())

allbus2006 |> 
  group_by(v83, meinung_personal) |> 
  summarise(n())

# Index

allbus2006$mein_sdperception <- rowMeans(allbus2006[,c("meinung_wirt", 
                                                "meinung_eltern", 
                                                "meinung_personal")], na.rm = TRUE)

allbus2006 |>
  select(mein_sdperception) |> 
  summary()

allbus2006 |> 
  select(meinung_wirt, meinung_eltern, meinung_personal, 
         mein_sdperception)

# Index standardisieren

allbus2006 <- allbus2006 |> 
  mutate(mein_sdperception = (mein_sdperception - min(mein_sdperception, na.rm = TRUE)) / 
           (max(mein_sdperception, na.rm = TRUE) - min(mein_sdperception, na.rm = TRUE)))
...
```{r m3}
m3.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split, data = allbus2006)

summary(m3.lm)
```

```{r m4}
m4.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split + sdtendency + mein_sdperception + wert_sdperception, data = allbus2006)
...
modelsummary::modelplot(list(m1.lm, m2.lm),
                        background = list(geom_vline(xintercept = 0, color = "darkgrey"))) + 
  aes(color = estimate) +
  scale_color_manual(values = c(colours_colourblind[1], colours_colourblind[2])) +
  ggtitle("Koeffizientenplot") +
  labs(
    colour = "Schätzer"
  )
...
modelsummary::modelplot(list(m1.lm, m2.lm, m3.lm),
                        background = list(geom_vline(xintercept = 0, color = "darkgrey"))) + 
  aes(color = estimate) +
  scale_color_manual(values = c(colours_colourblind[1], colours_colourblind[2], colours_colourblind[4])) +
  ggtitle("Koeffizientenplot") +
  labs(
    colour = "Schätzer"
  )
...
```{r m1_nohilfe}
m1_nohilfe.lm <- lm(immigration ~ split, data = allbus2006_nohilfe)
summary(m1_nohilfe.lm) # casi
```

```{r m2_nohilfe}
m2_nohilfe.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split, data = allbus2006_nohilfe)

summary(m2_nohilfe.lm)
```

```{r m3_nohilfe}
m3_nohilfe.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split, data = allbus2006_nohilfe)

summary(m3_nohilfe.lm)
```

```{r m4_nohilfe}
m4_nohilfe.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split + sdtendency + mein_sdperception + wert_sdperception, data = allbus2006_nohilfe)

summary(m4_nohilfe.lm)
```

```{r modelsummary_nohilfe}
modelsummary::modelsummary(list(m1_nohilfe.lm, m2_nohilfe.lm, m3_nohilfe.lm, m4_nohilfe.lm),  stars = T)
...
```{r m1_capi}
m1_capi.lm <- lm(immigration ~ split, data = allbus2006_capi)
summary(m1_capi.lm) # casi
```

```{r m2_capi}
m2_capi.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split, data = allbus2006_capi)

summary(m2_capi.lm)
```

```{r m3_capi}
m3_capi.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split, data = allbus2006_capi)

summary(m3_capi.lm)
```

```{r m4_capi}
m4_capi.lm <- lm(immigration ~ split + age_cen + age_cen * split + edu + edu * split + sex + sex * split + int_sex +
                                     int_sex * split + int_agediff + int_agediff * split + int_agecen + int_agecen * split + sdtendency + mein_sdperception + wert_sdperception, data = allbus2006_capi)

summary(m4_capi.lm)
```

```{r modelsummary_capi}
modelsummary::modelsummary(list(m1_capi.lm, m2_capi.lm, m3_capi.lm, m4_capi.lm),  stars = T)

modelsummary::modelsummary(list(m1_capi.lm, m2_capi.lm, m3_capi.lm, m4_capi.lm),  stars = T, output = "markdown", statistic = 'p.value')


modelsummary::modelplot(list(m1_capi.lm, m2_capi.lm, m3_capi.lm, m4_capi.lm),
                        background = list(geom_vline(xintercept = 0, color = "darkgrey"))) + 
  aes(color = estimate) +
  scale_color_manual(values = c(colours_colourblind[1], colours_colourblind[2], colours_colourblind[4], colours_colourblind[7])) +
  ggtitle("Koeffizientenplot") +
