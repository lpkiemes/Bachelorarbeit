...
pacman::p_load(haven, readr, tidyverse, gtsummary, ggsci)
...
allbus2006 <- allbus2006 |> 
  mutate(
    ...
    edu = case_when(v175 %in% c(1:5) ~ v175,
                    TRUE ~ NA),
...
```{r}
allbus2006 |> 
  group_by(split_selbst) |> 
  summarize(mean(immigration, na.rm = TRUE))
```
...
```{r}
split_immigration_multiple.lm <- lm(immigration ~ split_selbst + age_cen + edu + sex +interviewer_sex + interviewer_agediff, data = allbus2006)
